...
open class MusicPlayerUnitTests<T : Activity>(clazz: Class<T>) : AbstractUnitTest<T>(clazz) {
    ...
    fun View.assertBackgroundColor(errorMessage: String, @ColorInt expectedBackgroundColor: Int) {
        ...
        assertTrue(
            "Expected background to be ColorDrawable but was not. $errorMessage",
            this.background is ColorDrawable
        )
    ...
    fun List<Int>.clickSongSelectorListItems(
    ...
    fun addPlaylist(
        ...
        addPlaylistButtonOk.clickAndRun(0) // might or might not increase clock time
        assertViewStateIsPlayMusicState(
            songListView, fragmentContainer,
    ...
    fun loadPlaylist(
        menuItemIdLoadPlaylist: String,
        expectedPlaylistNameList: List<String>,
        playlistToLoadIndex: Int
    ): Int {
        val timeBefore = SystemClock.currentGnssTimeClock().millis()
        ...
        getLastAlertDialogWithShadow(
            ...
            shadowDialog.clickAndRunOnItem(
                playlistToLoadIndex,
                millis = 0
            ) // might or might not increase clock time
        }
        val timeAfter = SystemClock.currentGnssTimeClock().millis()
        return (timeAfter - timeBefore).toInt()
    }
